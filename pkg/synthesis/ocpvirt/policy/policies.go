...
func (np *PolicyGenerator) setAdminNetworkPolicy(
	...
	logging.Debugf("setAdminNetworkPolicy with srcSelector: %s, dstSelector %s  ", srcSelector.string(), dstSelector.string())
	...
	logging.Debugf("res admin subjet ns selector: %s", ns)
	logging.Debugf("res admin subjet pod selector: %s", pods)
}
