...
func (np *PolicyGenerator) addDefaultDenyNetworkPolicy() {
	...
	for _, namespace := range np.NamespacesInfo.Namespaces {
		policy := newNetworkPolicy("default-deny", namespace.Name, "Default Deny Network Policy", ruleID, ingressAndEgressType)
...
func (np *PolicyGenerator) addNetworkPolicy(srcSelector, dstSelector *policySelector,
	...
	newPolicyFunc := func(namespace string, typeValue policyType) *networkingv1.NetworkPolicy {
		policy := newNetworkPolicy(fmt.Sprintf("policy-%d", len(np.NetworkPolicies)), namespace, description, nsxRuleID, typeValue)
		np.NetworkPolicies = append(np.NetworkPolicies, policy)
		return policy
	}
...
func (np *PolicyGenerator) addDNSAllowNetworkPolicy() {
	for _, namespace := range np.NamespacesInfo.Namespaces {
		policy := newNetworkPolicy("dns-policy", namespace.Name, "Network Policy To Allow Access To DNS Server", noNSXRuleID, egressType)
		np.NetworkPolicies = append(np.NetworkPolicies, policy)
