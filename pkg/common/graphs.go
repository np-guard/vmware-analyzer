...
func (n *dotNode) string() string {
	return fmt.Sprintf("node_%d_[shape=box, label=%q, tooltip=%q fontcolor=darkgreen color=darkred]", n.ID, n.Kind()+":"+n.Name(), n.Name())
}
...
func (e *dotEdge) string() string {
	...
	if e.label != nil {
		s += fmt.Sprintf("[label=%q, tooltip=%q, labeltooltip=%q fontcolor=purple color=darkblue]", e.label.String(), e.label.String(), e.label.String())
	}
	return s
}
